...
type CustomerOrderPayload = {
  ...
  phoneCountryMode?: "by" | "other";
};

type CustomerOrderRow = {
  id: string;
  /** node-pg может отдать bigint строкой */
  order_number: number | string;
  phone: string;
  address: string;
  order_payload: CustomerOrderPayload;
  created_at: string;
  status?: string;
};

type CustomerOrderStatus = "new" | "processing" | "completed";

const CUSTOMER_ORDER_STATUS_OPTIONS: Array<{ value: CustomerOrderStatus; label: string }> = [
  { value: "new", label: "Новый" },
  { value: "processing", label: "В обработке/пути" },
  { value: "completed", label: "Завершён" },
];

function normalizeCustomerOrderStatus(raw: unknown): CustomerOrderStatus {
  if (raw === "processing" || raw === "completed") return raw;
  return "new";
}

function customerOrderStatusLabel(status: CustomerOrderStatus): string {
  switch (status) {
    case "new":
      return "Новый";
    case "processing":
      return "В обработке/пути";
    case "completed":
      return "Завершён";
    default: {
...
function customerOrderStatusBadgeClass(status: CustomerOrderStatus): string {
  switch (status) {
    case "new":
      return "bg-slate-200 text-slate-800 dark:bg-slate-600 dark:text-slate-100";
    case "processing":
      return "bg-amber-100 text-amber-900 dark:bg-amber-900/45 dark:text-amber-50";
    case "completed":
...
function formatCustomerOrderDate(iso: string) {
  try {
...
function Admin() {
  ...
  const [isAuthed, setIsAuthed] = useState(() => {
    if (!getToken(ACCESS_TOKEN_KEY) && !getToken(REFRESH_TOKEN_KEY)) return false;
    return getIsAuthed();
  });
  ...
  const [editingCustomerOrderStatusId, setEditingCustomerOrderStatusId] = useState<string | null>(null);
  const [patchingCustomerOrderNumber, setPatchingCustomerOrderNumber] = useState<number | null>(null);
  ...
  const loadCustomerOrders = async () => {
    ...
    try {
      const data = await adminFetchJson<{ ok: boolean; items: CustomerOrderRow[] }>("/api/admin/customer-orders");
      setCustomerOrders(data.items ?? []);
    } catch {
      setCustomerOrdersError("Не удалось загрузить заказы.");
      setCustomerOrders([]);
    } finally {
  ...
  const patchCustomerOrderStatus = async (
    orderNumber: number | string,
    status: CustomerOrderStatus,
  ): Promise<boolean> => {
    const num = typeof orderNumber === "string" ? Number.parseInt(orderNumber, 10) : orderNumber;
    if (!Number.isFinite(num)) return false;
    setPatchingCustomerOrderNumber(num);
    setCustomerOrdersError(null);
    try {
      await adminFetchJson<{ ok: boolean }>(`/api/admin/customer-orders/${num}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      setCustomerOrders((prev) =>
        prev.map((o) =>
          String(o.order_number) === String(num) ? { ...o, status } : o,
        ),
      );
      return true;
    } catch {
      setCustomerOrdersError("Не удалось сохранить статус заказа.");
      void loadCustomerOrders();
      return false;
    } finally {
      setPatchingCustomerOrderNumber(null);
    }
  };
  ...
  useEffect(() => {
    ...
    void loadCustomerOrders();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeTab, isAuthed, isCheckingAuth]);

  useEffect(() => {
    if (activeTab !== "orders") {
      setExpandedCustomerOrderId(null);
      setEditingCustomerOrderStatusId(null);
    }
  }, [activeTab]);
  ...
  if (!isAuthed) {
    const verdantMeshBg: CSSProperties = {
          ...
          <main className="flex-1 overflow-y-auto overflow-x-hidden">
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div className="relative w-full max-w-2xl rounded-2xl border border-slate-200 bg-white shadow-2xl animate-[fadeIn_180ms_ease-out]">
                  ...
                  <div className="p-4 space-y-6 max-h-[70vh] overflow-auto">
                    {isSearching ? <div className="text-sm text-slate-500">Поиск...</div> : null}
            ...
            <div className="mx-auto max-w-6xl space-y-8 p-4 sm:p-6 lg:p-8">
              {activeTab === "dashboard" ? (
                ...
                <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
                  <div className="px-4 py-3 sm:px-5 border-b border-slate-100 dark:border-slate-800">
                    <h3 className="text-lg font-bold">Заказы с сайта</h3>
                    <p className="text-slate-500 text-xs">
                      Нажмите на строку, чтобы открыть полные данные. Новые сверху.
                    </p>
                  </div>
                  <div className="p-3 sm:p-4">
                    {customerOrdersError ? (
                      <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-200 mb-3">
                        {customerOrdersError}
                      </div>
                    ) : null}
                    {isLoadingCustomerOrders && customerOrders.length === 0 ? (
                      <p className="text-sm text-slate-500">Загрузка заказов…</p>
                    ) : customerOrders.length === 0 ? (
                      <p className="text-sm text-slate-600 dark:text-slate-300">
                        Пока нет заявок. Оформленный с сайта заказ появится здесь.
                      </p>
                    ) : (
                      <ul className="rounded-lg border border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-800 overflow-hidden">
                        {customerOrders.map((order) => {
                          const p = order.order_payload ?? {};
                          const items = Array.isArray(p.items) ? p.items : [];
                          const subtotal = typeof p.subtotal === "number" ? p.subtotal : null;
                          const delivery = typeof p.delivery === "number" ? p.delivery : null;
                          const discount = typeof p.discount === "number" ? p.discount : null;
                          const total = typeof p.total === "number" ? p.total : null;
                          const promo = typeof p.promoCode === "string" && p.promoCode.trim() ? p.promoCode.trim() : null;
                          const phoneMode = p.phoneCountryMode === "other" ? "other" : "by";
                          ...
                          const isPatchingThis =
                            patchingCustomerOrderNumber != null &&
                            String(patchingCustomerOrderNumber) === String(order.order_number);
                          return (
                            <li key={`${order.order_number}-${order.id}`} className="bg-white dark:bg-slate-900/30">
                              <div
                                className={[
                                  "flex min-w-0 items-stretch",
                                  isOpen ? "bg-primary/10 dark:bg-primary/10" : "",
                                ].join(" ")}
                              >
                                <button
                                  className={[
                                    "flex min-w-0 flex-1 items-center gap-2 sm:gap-3 px-2.5 py-2 sm:px-3 sm:py-2.5 text-left text-xs sm:text-sm transition-colors",
                                    "hover:bg-slate-50 dark:hover:bg-slate-800/60",
                                  ].join(" ")}
                                  type="button"
                                  aria-expanded={isOpen}
                                  aria-label={isOpen ? "Свернуть детали заказа" : "Показать полные данные заказа"}
                                  onClick={() => {
                                    setExpandedCustomerOrderId((id) => (id === order.id ? null : order.id));
                                    setEditingCustomerOrderStatusId(null);
                                  }}
                                >
                                  <span className="font-bold tabular-nums text-primary shrink-0 w-14 sm:w-16">
                                    №{order.order_number}
                                  </span>
                                  <span className="text-slate-500 shrink-0 tabular-nums text-[10px] sm:text-xs w-[6.75rem] sm:w-[8.5rem]">
                                    {formatCustomerOrderDate(order.created_at)}
                                  </span>
                                  <span className="min-w-0 flex-1 truncate text-slate-800 dark:text-slate-200 font-medium">
                                    {order.phone}
                                  </span>
                                  <span className="text-slate-500 shrink-0 tabular-nums hidden md:inline w-12 text-center">
                                    {items.length} поз.
                                  </span>
                                  <span className="font-semibold tabular-nums shrink-0 text-slate-900 dark:text-slate-100 w-[4.5rem] sm:w-[5.25rem] text-right">
                                    {total != null ? `${total.toFixed(2)}` : "—"}
                                  </span>
                                  <span className="text-[10px] text-slate-400 shrink-0 hidden lg:inline w-8">BYN</span>
                                  <IconChevronDown
                                    className={[
                                      "h-4 w-4 sm:h-5 sm:w-5 shrink-0 text-slate-400 transition-transform",
                                      isOpen ? "rotate-180" : "",
                                    ].join(" ")}
                                  />
                                </button>
                                <div className="flex shrink-0 items-center gap-0.5 px-0.5 sm:px-1">
                                  <span
                                    className={[
                                      "shrink-0 rounded px-1.5 py-0.5 text-[10px] font-bold max-w-[4.5rem] truncate sm:max-w-[7rem]",
                                      customerOrderStatusBadgeClass(orderStatus),
                                      isPatchingThis ? "opacity-60" : "",
                                    ].join(" ")}
                                    title={customerOrderStatusLabel(orderStatus)}
                                  >
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.preventDefault();
                                      e.stopPropagation();
                                      setEditingCustomerOrderStatusId((id) =>
                              ...
                              {editingCustomerOrderStatusId === order.id ? (
                                <div className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-3 py-2">
                                  <label className="sr-only" htmlFor={`order-status-${order.id}`}>
                                    Статус заказа
                                  </label>
                                  <select
                                    id={`order-status-${order.id}`}
                                    className="w-full max-w-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm py-1.5 px-2 text-slate-900 dark:text-slate-100"
                                    disabled={isPatchingThis}
                                    value={orderStatus}
                                    onChange={(e) => {
                                      const v = e.target.value;
                                      if (v === "new" || v === "processing" || v === "completed") {
                                        void (async () => {
                                          const ok = await patchCustomerOrderStatus(order.order_number, v);
                                          if (ok) setEditingCustomerOrderStatusId(null);
                                        })();
                                      }
                                    }}
                                  >
                                    {CUSTOMER_ORDER_STATUS_OPTIONS.map((o) => (
                              ...
                              {isOpen ? (
                                <div className="border-t border-slate-100 dark:border-slate-800 px-3 pb-3 pt-2 space-y-3 text-sm bg-slate-50/70 dark:bg-slate-900/50">
                                  <p className="text-xs text-slate-500">
                                    <span className="font-medium text-slate-600 dark:text-slate-400">Дата: </span>
                                    {formatCustomerOrderDate(order.created_at)}
                                  </p>
                                  <p className="text-xs text-slate-500">
                                    Режим телефона: {phoneMode === "other" ? "другой" : "Беларусь"}
                                  </p>
                                  <div className="grid gap-2 sm:grid-cols-2">
                                    <div>
                                      <p className="text-xs text-slate-500 mb-0.5">Телефон</p>
                                      <p className="font-medium text-slate-900 dark:text-slate-100 [overflow-wrap:anywhere]">
                                        {order.phone}
                                      </p>
                                    </div>
                                    <div className="sm:col-span-2">
                                      <p className="text-xs text-slate-500 mb-0.5">Адрес</p>
                                      <p className="font-medium text-slate-900 dark:text-slate-100 whitespace-pre-wrap [overflow-wrap:anywhere]">
                                        {order.address}
                                      </p>
                                    </div>
                                  </div>
                                  {items.length > 0 ? (
                                    <div>
                                      <p className="text-xs text-slate-500 mb-1.5 font-medium">Состав</p>
                                      <div className="rounded-lg border border-slate-200 dark:border-slate-600 overflow-x-auto bg-white dark:bg-slate-900/40">
                                        <table className="w-full text-xs sm:text-sm min-w-[280px]">
                                          <thead className="bg-slate-100 dark:bg-slate-800/80 text-left text-slate-500">
                                            <tr>
                                              <th className="px-2 py-1.5 sm:px-3 sm:py-2 font-medium">Товар</th>
                                              <th className="px-2 py-1.5 sm:px-3 sm:py-2 font-medium tabular-nums w-[3.5rem]">
                                                Кол-во
                                              </th>
                                              <th className="px-2 py-1.5 sm:px-3 sm:py-2 font-medium tabular-nums w-[4rem]">
                                                Цена
                                              </th>
                                              <th className="px-2 py-1.5 sm:px-3 sm:py-2 font-medium tabular-nums text-right w-[5rem]">
                                                Сумма
                                              </th>
                                            </tr>
                                          </thead>
                                          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                            {items.map((line) => {
                                              const lineSum = Number(line.price) * Number(line.quantity);
                                              return (
                                                <tr key={`${order.id}-${line.id}`}>
                                                  <td className="px-2 py-1.5 sm:px-3 sm:py-2 [overflow-wrap:anywhere]">
                                                    <span className="font-medium text-slate-900 dark:text-slate-100">
                                                      {line.name}
                                                    </span>
                                                    {line.subtitle ? (
                                                      <span className="block text-[11px] text-slate-500">{line.subtitle}</span>
                                                    ) : null}
                                                  </td>
                                                  <td className="px-2 py-1.5 sm:px-3 sm:py-2 tabular-nums">
                                                    {line.quantity}
                                                  </td>
                                                  <td className="px-2 py-1.5 sm:px-3 sm:py-2 tabular-nums">
                                                    {Number(line.price).toFixed(2)}
                                                  </td>
                                                  <td className="px-2 py-1.5 sm:px-3 sm:py-2 tabular-nums text-right font-medium">
                                                    {Number.isFinite(lineSum) ? lineSum.toFixed(2) : "—"}
                                                  </td>
                                                </tr>
                                              );
                                            })}
                                  ...
                                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-600 dark:text-slate-400">
                                    ...
                                    {delivery != null ? (
                                      <span>
                                        ...
                                        <span className="font-semibold tabular-nums">
                                          {delivery === 0 ? "0" : delivery.toFixed(2)}
                                        </span>
                                      </span>
                                    ) : null}
                                    ...
                                    {promo ? (
                                      <span>
                                        Промокод: <span className="font-mono font-semibold">{promo}</span>
                                      </span>
                                    ) : null}
                                    {total != null ? (
                                      <span className="font-bold text-primary">
                                        Итого:{" "}
                                        <span className="tabular-nums">{total.toFixed(2)} BYN</span>
                                      </span>
                                    ) : null}
                                  </div>
                                </div>
